...
class Responses:
    @staticmethod
    def failure(code, alert_msg_description, alert_msg_type, from_cache=0, status=None, result=None, error_code=None,
                is_access_log_required=False):
        """
        This api is to return json serialized response in 
        """
        is_data = 1 if result else 0
        description = alert_msg_description if int(DEBUG_MODE) == 1 else common["FAILURE_MSG"]
    ...
    @staticmethod
    def success(code, alert_msg_description, alert_msg_type, from_cache=0, result=None, status=None):
        """This api is to return json serialized response in success case"""
        is_data = 1 if result else 0
        description = alert_msg_description if int(DEBUG_MODE) == 1 else common["SUCCESS_MSG"]
        resp = {
            "response": {
                "code": code,
                "error_code": 0,
